class Client < ApplicationRecord
  ...
  def active_for_authentication?
    super && check_if_company_is_active && domain_correct?
  end
  ...
  def check_if_company_is_active
    Company.find(company_id).ativo?
  end

  def domain_correct?
    client_register_email = email.split('@').last
    valid_company_find = Company.find(company_id)
    company_domain_pattern = valid_company_find.company_admin_email.split('@').last

    client_register_email == company_domain_pattern
  end
end
